...
════════════════════════════════════════════════════════════════════
PROMPT 4 — THE GOAL LOOP  (GLM-5.2 — paste verbatim to start the autonomous audit→test→fix→re-test)
════════════════════════════════════════════════════════════════════
Load skill loop-runner and skill agent-stack. Enter [LOOP] mode. Checkpoint and WAIT between
every phase — do not auto-advance.

/goal Go over every single feature in this app. Create a user story with expected behaviour
derived FROM THE CODE for each. Keep a single canonical tracker, FEATURES.md (+ FEATURES.csv),
tracking every feature's status. When Phase 1 is done, stop. On "continue", switch the loop to
testing every user story and documenting every error. On "continue", fix every logic error and
every UX error with the smallest diff per fix. On "continue", re-test every user behaviour
post-fix as a regression pass. Back every status change with evidence (a test pass or a
rendered route), never assertion. If you get lost in the woods, stop and ask.

Begin Phase 1 (AUDIT & SPEC) now. Build FEATURES.md, then checkpoint and wait.

Drive it with:  continue · status · redo <id> · stop
