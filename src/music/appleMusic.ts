// FILE: src/music/appleMusic.ts
// Apple Music account connection for the sound dock, through MusicKit on the
// Web (v3). MusicKit needs a developer token signed with the store's private
// MusicKit key, so that is minted server-side by the Netlify function
// `apple-music-token`; this file only ever sees the signed token.

const MUSICKIT_URL = 'https://js-cdn.music.apple.com/musickit/v3/musickit.js'
const TOKEN_ENDPOINT = '/.netlify/functions/apple-music-token'

let token: Promise<string | null> | null = null

/**
 * The developer token, or null when the store has not configured Apple Music.
 * A plain `vite` dev server (and the SPA fallback) answers unknown paths with
 * index.html, so only a JSON body counts as an answer.
 */
export function fetchAppleDeveloperToken(): Promise<string | null> {
  return (token ??= (async () => {
    ...
    // Do not remember a failure: the next open of the dock asks again.
    if (!t) token = null
    return t
  }))
}
 ...
 * this store. Prepared ahead of the click: `authorize()` opens Apple's sign-in
...
export function prepareAppleMusic(): Promise<MusicKitWeb.Instance | null> {
  return (instance ??= (async () => {
    ...
    return musicKit.configure({ developerToken, app: { name: 'Buddington', build: '1.0.0' } })
  })().then(
    (inst) => {
...
export async function listAppleLibraryPlaylists(music: MusicKitWeb.Instance): Promise<ApplePlaylist[]> {
  ...
  for (const raw of rows) {
    ...
    const row = raw as { id?: unknown; attributes?: { name?: unknown; artwork?: { url?: unknown } } }
