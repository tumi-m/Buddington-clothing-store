...
describe('pkce', () => {
  ...
  it('builds the authorize url Spotify expects', () => {
    const url = new URL(
      authorizeUrl({
        clientId: 'client-1',
        redirectUri: 'https://shop.example/',
        challenge: 'chal',
        state: 'st8',
      }),
    )
    ...
    assert.deepEqual(Object.fromEntries(url.searchParams), {
      ...
      code_challenge: 'chal',
      redirect_uri: 'https://shop.example/',
      state: 'st8',
    })
  })
  ...
  it('builds the code exchange and refresh bodies', () => {
    assert.deepEqual(
      ...
      { client_id: 'c', grant_type: 'authorization_code', code: 'k', redirect_uri: 'https://s/', code_verifier: 'v' },
    )
...
describe('tokens', () => {
  ...
  it('keeps the old refresh token when a refresh does not rotate it', () => {
    assert.equal(readTokenResponse({ access_token: 'a2', expires_in: 60 }, 0, 'r1')?.refreshToken, 'r1')
  })
