// FILE: src/music/useSpotifySession.ts
// Lifecycle of a connected Spotify account: finish the sign-in redirect, then —
// only once the shopper opens the sound dock — load the Web Playback SDK and
...
import type { NowPlaying, SessionStatus } from './types'
...
function messageFor(err: unknown): string {
  ...
  switch (code) {
    case 'premium-required':
      return 'Spotify Premium is needed to play inside the store. Paste a link below to use Spotify’s own player instead.'
    case 'signed-out':
      return 'Your Spotify session ended. Connect again.'
    case 'no-device':
      return 'Spotify lost this browser as a player. Close and reopen Sound to reconnect.'
    case 'list-failed':
...
export function useSpotifySession(wanted: boolean): SpotifySession {
  const [status, setStatus] = useState<SessionStatus>(() =>
    !spotifyConfigured ? 'off' : hasSpotifySession() ? 'starting' : 'signed-out',
  )
    ...
    loadSpotifySdk()
      .then((sdk) => {
        ...
        player.addListener('account_error', () =>
          fail('Spotify Premium is needed to play inside the store. Paste a link below to use Spotify’s own player instead.'),
        )
        ...
        player.addListener('playback_error', () => setMessage('Spotify couldn’t play that. Try another.'))
        player.addListener('autoplay_failed', () => setMessage('Your browser held back autoplay. Press play.'))
      ...
      .catch(() => {
        if (!cancelled) fail('Spotify’s player couldn’t load. A content blocker or the network may be stopping it.')
      })
  ...
  return useMemo(
    () => ({ status, message, playlists, nowPlaying, returned, connect, disconnect, play, toggle, next, pause }),
    [status, message, playlists, nowPlaying, returned, connect, disconnect, play, toggle, next, pause],
  )
}
