// FILE: src/music/useAppleMusicSession.ts
// Lifecycle of a connected Apple Music account through MusicKit. Like Spotify,
// nothing loads until the shopper opens the sound dock; unlike Spotify the
...
import { listAppleLibraryPlaylists, prepareAppleMusic, type ApplePlaylist } from './appleMusic'
...
export function useAppleMusicSession(wanted: boolean): AppleSession {
  ...
  useEffect(() => {
    ...
    prepareAppleMusic().then(
      ...
      () => {
        if (cancelled) return
        setStatus('error')
        setMessage('Apple Music couldn’t load. A content blocker or the network may be stopping it.')
      },
    )
    return () => {
      cancelled = true
  ...
  return useMemo(
    () => ({ status, message, previewOnly, playlists, nowPlaying, connect, disconnect, play, toggle, next, pause }),
    [status, message, previewOnly, playlists, nowPlaying, connect, disconnect, play, toggle, next, pause],
  )
}
