// FILE: src/music/spotify.ts
// Spotify account connection for the sound dock: PKCE sign-in, token refresh,
// the Web Playback SDK (which turns this tab into a Spotify device), and the
// two Web API calls the dock needs. Browser-only; the pure pieces are in pkce.ts.
...
const TOKENS_KEY = 'buddington-spotify'
const PENDING_KEY = 'buddington-spotify-pending'
const SDK_URL = 'https://sdk.scdn.co/spotify-player.js'
const API = 'https://api.spotify.com/v1'

export const spotifyClientId = (import.meta.env.VITE_SPOTIFY_CLIENT_ID ?? '').trim()
/** Without a client id there is no way to connect, so the dock hides the option. */
...
export async function beginSpotifyConnect(): Promise<void> {
  ...
  window.location.assign(
    authorizeUrl({ clientId: spotifyClientId, redirectUri: spotifyRedirectUri(), challenge, state }),
  )
}
...
async function runCallback(): Promise<CallbackResult> {
  ...
  if (error) {
    return {
      kind: 'error',
      message: error === 'access_denied' ? 'Spotify connection was cancelled.' : 'Spotify could not connect.',
    }
  }
...
export async function listSpotifyPlaylists(): Promise<SpotifyPlaylist[]> {
  ...
  for (const raw of body.items ?? []) {
    ...
    const p = raw as { id?: unknown; name?: unknown; uri?: unknown; images?: { url?: unknown }[] | null }
    if (typeof p.id !== 'string' || typeof p.name !== 'string' || typeof p.uri !== 'string') continue
    const image = p.images?.[0]?.url
    out.push({ id: p.id, name: p.name, uri: p.uri, image: typeof image === 'string' ? image : undefined })
  }
  return out
}
