// FILE: src/lib/cutout.ts
// Shared background-removal ("cutout") used by both the 3D cloth texture and the
// experience carousel thumbnails, so a garment reads as a free-floating render
// everywhere — never a photo on a rectangular box.
//
// Method: a gradient-aware region grow from the image borders. A pixel joins the
// background when it is close to its already-background neighbour (so soft studio
// gradients are followed) AND still within HARD_TOL of the overall border tone
// (so the grow can't walk deep into a same-edge garment). The cut alpha is
// feathered 1px to avoid aliasing. If "background" would swallow the garment the
// build bails (returns null) and callers fall back to the raw image.

const WORK_W = 320           // working resolution for the mask (one-time cost)
const LOCAL_TOL = 32         // neighbour-to-neighbour colour distance the grow may cross
const HARD_TOL = 140         // max distance from the border tone — gates leaks into the garment
const MAX_BG_FRACTION = 0.93 // safety: if "background" swallows the garment, bail

export function buildCutoutCanvas(img: HTMLImageElement): HTMLCanvasElement | null {
  const ratio = img.naturalHeight / img.naturalWidth
  const w = Math.min(WORK_W, img.naturalWidth)
  const h = Math.max(1, Math.round(w * ratio))

  const canvas = document.createElement('canvas')
  canvas.width = w
  canvas.height = h
  const ctx = canvas.getContext('2d', { willReadFrequently: true })
  if (!ctx) return null
  ctx.drawImage(img, 0, 0, w, h)

  let imgData: ImageData
  try { imgData = ctx.getImageData(0, 0, w, h) } catch { return null }
  const data = imgData.data
  const n = w * h

  // Border-average colour (the presumed background tone).
  let br = 0, bg = 0, bb = 0, bc = 0
  const sampleBorder = (x: number, y: number) => {
    const i = (y * w + x) * 4
    br += data[i]; bg += data[i + 1]; bb += data[i + 2]; bc++
  }
  for (let x = 0; x < w; x++) { sampleBorder(x, 0); sampleBorder(x, h - 1) }
  for (let y = 0; y < h; y++) { sampleBorder(0, y); sampleBorder(w - 1, y) }
  br /= bc; bg /= bc; bb /= bc

  const local2 = LOCAL_TOL * LOCAL_TOL
  const hard2  = HARD_TOL * HARD_TOL
  const mask = new Uint8Array(n)       // 1 = background
  const stack = new Int32Array(n)
  let sp = 0

  // Seed the whole outer ring as background.
  for (let x = 0; x < w; x++) {
    const top = x, bot = (h - 1) * w + x
    mask[top] = 1; stack[sp++] = top
    mask[bot] = 1; stack[sp++] = bot
  }
  for (let y = 0; y < h; y++) {
    const left = y * w, right = y * w + (w - 1)
    if (!mask[left])  { mask[left]  = 1; stack[sp++] = left }
    if (!mask[right]) { mask[right] = 1; stack[sp++] = right }
  }

  const tryGrow = (from: number, to: number) => {
    if (mask[to]) return
    const ti = to * 4, fi = from * 4
    const dl = (data[ti] - data[fi]) ** 2 + (data[ti + 1] - data[fi + 1]) ** 2 + (data[ti + 2] - data[fi + 2]) ** 2
    if (dl >= local2) return
    const dh = (data[ti] - br) ** 2 + (data[ti + 1] - bg) ** 2 + (data[ti + 2] - bb) ** 2
    if (dh >= hard2) return
    mask[to] = 1; stack[sp++] = to
  }

  while (sp > 0) {
    const idx = stack[--sp]
    const x = idx % w
    const y = (idx / w) | 0
    if (x > 0)     tryGrow(idx, idx - 1)
    if (x < w - 1) tryGrow(idx, idx + 1)
    if (y > 0)     tryGrow(idx, idx - w)
    if (y < h - 1) tryGrow(idx, idx + w)
  }

  // Safety bail-out: never erase the whole garment.
  let bgCount = 0
  for (let i = 0; i < n; i++) bgCount += mask[i]
  if (bgCount / n > MAX_BG_FRACTION) return null

  // Feathered alpha: 1px smoothing across the foreground/background boundary.
  for (let i = 0; i < n; i++) {
    const x = i % w
    const y = (i / w) | 0
    let fg = 0, tot = 0
    for (let dy = -1; dy <= 1; dy++) {
      for (let dx = -1; dx <= 1; dx++) {
        const xx = x + dx, yy = y + dy
        if (xx < 0 || yy < 0 || xx >= w || yy >= h) continue
        tot++
        if (!mask[yy * w + xx]) fg++
      }
    }
    data[i * 4 + 3] = Math.round((fg / tot) * 255)
  }

  ctx.putImageData(imgData, 0, 0)
  return canvas
}

// ── Cached PNG data-URL cutouts for HTML thumbnails (the carousel) ────────────
const urlCache = new Map<string, Promise<string>>()

export function loadCutoutURL(path: string): Promise<string> {
  const cached = urlCache.get(path)
  if (cached) return cached
  const p = new Promise<string>((resolve) => {
    const img = new Image()
    img.crossOrigin = 'anonymous'
    img.decoding = 'async'
    img.onload = () => {
      const canvas = buildCutoutCanvas(img)
      try { resolve(canvas ? canvas.toDataURL('image/png') : path) }
      catch { resolve(path) }
    }
    img.onerror = () => resolve(path)
    img.src = path
  })
  urlCache.set(path, p)
  return p
}
