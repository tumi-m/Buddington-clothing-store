    ...
    return (
      <div className="relative w-full h-full bg-dark-bg select-none">
        ...
        <button
          ...
          className="absolute top-6 left-1/2 -translate-x-1/2 z-50 font-mono uppercase text-[0.65rem] tracking-[0.14em] text-gray-500 hover:text-gold border border-white/10 hover:border-gold px-3 py-1.5 rounded transition-colors"
        >
          ← EXIT TO SITE
        </button>
      </div>
  ...
  return (
    <div className="absolute inset-0 overflow-y-auto bg-paper text-ink">
      ...
      <main>
        ...
        {view === 'shop' && <Shop onOpenProduct={openProduct} onNavigate={navigate} />}
        {view === 'product' && product && (
          <ProductDetail product={product} onBack={() => navigate('shop')} />
        )}
        {view === 'product' && !product && (
